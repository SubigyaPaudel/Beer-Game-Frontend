...
const DefaultGame = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setDistributorPresent(dummy_data.distributor_present === "True");
      setWholesalerPresent(dummy_data.wholesaler_present === "True");
      setplaceholders(dummy_data); //corresponds to that of a dummy game
      set_demand_patterns(dummy_demand_patterns);
      setStudents(local_students);
      setLoading(false);
      setErrors(false);
    }, 500);
    setTimeout(console.log(my_demand_patterns), 5000);
    return () => {
      abortConst.abort();
    };
  }, []);
  ...
  return (
    <>
      ...
      >
        ...
        {success && (
          <h5>The default game parameters have been successfully uploaded</h5>
        )}
        <Form.Row>
          ...
          <Form.Group as={Col} md="6">
            <Form.Label>Default Demand Pattern</Form.Label>
